select
    ...
    uniqExactIf((subscription_id, product_id), subscribed_dt < exp_start_dt and trial > 0 and not (toDate(charge_dt) = toDate(subscribed_dt)) and toDateTime(first_charge_expected_dt) + interval 1 day > exp_start_dt) as active_trial_cnt,
    ...
    uniqExactIf((subscription_id, product_id), subscribed_dt < exp_start_dt and not (toDate(charge_dt) = toDate(subscribed_dt)) and toDateTime(charge_dt) between toDateTime(subscribed_dt) and toDateTime(first_charge_expected_dt) + interval 1 day) as active_charged_trial_cnt,
